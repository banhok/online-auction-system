 ...
 * vì JDBC Connection không thread-safe; xem note ở DatabaseManager.
 */
public class AuctionDAO {
    ...
    public Optional<Auction> findByItemId(int itemId) throws SQLException {
        synchronized (DatabaseManager.getDbLock()) {
    ...
    public List<Auction> findByWinnerId(int winnerId) throws SQLException {
        synchronized (DatabaseManager.getDbLock()) {
            String sql = "SELECT * FROM auctions WHERE winner_id = ? " +
                         "AND status IN ('FINISHED', 'PAID') ORDER BY updated_at DESC";
            List<Auction> auctions = new ArrayList<>();
    ...
    public List<Auction> findExpiredRunning() throws SQLException {
        synchronized (DatabaseManager.getDbLock()) {
            ...
            try (PreparedStatement ps = getConnection().prepareStatement(sql)) {
                ps.setString(1, DateTimeUtil.formatDb(LocalDateTime.now()));
                try (ResultSet rs = ps.executeQuery()) {
                    while (rs.next()) {
                        auctions.add(mapRowToAuction(rs));
                    }
    ...
    public void update(Auction auction) throws SQLException {
        synchronized (DatabaseManager.getDbLock()) {
            String sql = "UPDATE auctions SET current_price = ?, bid_count = ?, status = ?, " +
                         "end_time = ?, winner_id = ?, updated_at = datetime('now', 'localtime') WHERE id = ?";
    ...
    public void updateStatus(int auctionId, AuctionStatus status) throws SQLException {
        synchronized (DatabaseManager.getDbLock()) {
            String sql = "UPDATE auctions SET status = ?, updated_at = datetime('now', 'localtime') WHERE id = ?";
